...
class MSE(torch.nn.Module):
    ...
    def forward(self, X, Y):
        ...
        if self.normalization:
            X = (X + 1) / 2
            Y = (Y + 1) / 2
        return torch.mean(self.squared_difference(X * 255.0, Y * 255.0))
...
class Distortion(torch.nn.Module):
    def __init__(self, args):
        ...
        if args.distortion_metric == "MSE":
            ...
            # Inputs are typically in [0,1] (ToTensor), so use data_range=1.0.
            self.dist = SSIM(data_range=1.0)
        elif args.distortion_metric == "MS-SSIM":
            self.dist = MS_SSIM(data_range=1.0, levels=4, channel=3)
        else:
            raise ValueError(f"Unknown distortion metric: {args.distortion_metric!r}")
...
if __name__ == "__main__":
    rand_im1 = (torch.randint(0, 255, [4, 3, 256, 128], dtype=torch.float32) / 255.0).cuda()
    rand_im2 = (torch.randint(0, 255, [4, 3, 256, 128], dtype=torch.float32) / 255.0).cuda()
